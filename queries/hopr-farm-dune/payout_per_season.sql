...
WITH open_farm AS (
    ...
    SELECT * FROM until_ongoing_period
    ORDER BY season DESC
    LIMIT 1
), non_finalized_period AS (
    SELECT generate_series(season, 13) AS period, eligible_seeds, payout_per_uni
    FROM ongoing_period
), current_past_period AS (
    SELECT * FROM until_ongoing_period
    UNION
    SELECT * FROM non_finalized_period
)

SELECT * FROM current_past_period
ORDER BY season