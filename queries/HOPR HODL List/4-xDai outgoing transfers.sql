--Same code, differnet chain (xDai) and different contract_address (xHOPR)
...
SELECT Get_sum_out.address AS address, 
       Get_sum_out.sum_out AS value, 
       Get_Lastday.time AS time,
       Get_Lastday.evt_block_number AS block_number
FROM Get_sum_out 
JOIN Get_Lastday ON Get_sum_out.address = Get_Lastday.address
ORDER BY Get_Lastday.time ASC
